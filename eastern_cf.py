...
eastern_statefp_codes = [
    "01","05","09","10","12","13","19","17","18","21","22","25","24",
    "23","26","27","29","28","37","38","31","33","34","36","39","42",
    "44","45","46","47","51","50","55","54","20","40"
]
...
def filter_eastern_counties(shp_path):
    ...
    # Shapefile typically has a column like 'STATEFP' or 'STATEFP' for the state abbreviation
    # Many county shapefiles have 'STATEFP' as a 2-digit FIPS code, or 'STATEFP' as the 2-letter abbreviation.
    # We'll try 'STATEFP' first. If your file is different, adjust accordingly.
    col_candidates = [c.upper() for c in gdf.columns]
    if "STATEFP" in col_candidates:
        st_col = [c for c in gdf.columns if c.upper() == "STATEFP"][0]
        # Filter to counties that have STATEFP in our Eastern states list
        # Some shapefiles store DC as 'DC'; some store it as '11'; so watch out for special cases.
        # We'll just filter if the state is in eastern_statefp_codes
        gdf_filtered = gdf[gdf[st_col].isin(eastern_statefp_codes)]
    else:
        raise ValueError("Could not find STATEFP column in county shapefile. Please adjust the script to match your shapefile's state column.")
